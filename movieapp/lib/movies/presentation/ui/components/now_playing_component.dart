...
import 'package:movieapp/movies/presentation/ui/screens/movie_detail_screen.dart';
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MovieBloc, MovieState>(
      buildWhen: (previous, current) =>
          previous.nowPlayingDataState != current.nowPlayingDataState,
      builder: (context, state) {
        print('nowPlaying Bloc');
        switch (state.nowPlayingDataState) {
                    ...
                    return GestureDetector(
                      ...
                      onTap: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                MovieDetailScreen(id: item.movieId),
                          ),
                        );
                      },
                      child: Stack(
                        children: [
                          ShaderMask(
                            ...
                            child: CachedNetworkImage(
                              height: 560.0,
                              imageUrl: ApiConst.imageUrl(item.movieImage),
                              fit: BoxFit.cover,
                            ),
                          ),
                          Align(
          ...
          case RequestApiDataState.dataError:
            return SizedBox(
              height: 400,
              child: Center(
                child: Text(state.nowPlayingMessageState),
              ),
            );
        }
      },
